...
from _lib import (
    C_IN, C_OP, C_OUT, C_REG, C_SMEM, arrow, box, footer, label, new_fig, save,
    title,
)
...
def main():
    ...
    y8 = 6.2
    box(ax, 0.3, y8, 2.4, 1.0,
        "A_q [M, K]\nint8 activations\n(per-row scale)", C_IN, fontsize=9)
    box(ax, 2.9, y8, 2.4, 1.0,
        "B_q [N, K]\nint8 weights\n(pre-transposed)", C_IN, fontsize=9)
    box(ax, 5.5, y8, 2.4, 1.0,
        "smem As[64][32]\n    Bs[64][32]\nint8 tiles, 2KB each", C_SMEM, fontsize=9)
    box(ax, 8.1, y8, 2.4, 1.0,
        "wmma m16n16k16 int8\n4 warps × 4 n-tiles\nint32 accumulator", C_OP, fontsize=9)
    box(ax, 10.7, y8, 2.0, 1.0,
        "epilogue\n× a_scale[m] × b_scale[n]\n→ fp32 C", C_OUT, fontsize=9)
    ...
    label(ax, 6.5, 5.15,
          ...
          fontsize=10.5, color="#222")
    y4 = 3.6
    box(ax, 0.3, y4, 2.4, 1.0,
        "A [M, K]\nfp16 activations\n(no quant)", C_IN, fontsize=9)
    box(ax, 2.9, y4, 2.4, 1.0,
        "Wq [N, K/2] uint8 packed\nscales / zeros [K/G, N] fp16\n(one per group G=64)",
        C_IN, fontsize=8.5)
    box(ax, 5.5, y4, 2.4, 1.0,
        "smem As[64][64] fp16\n    Ws[64][64] fp16\n(dequantised in-register)",
        C_SMEM, fontsize=8.5)
    box(ax, 8.1, y4, 2.4, 1.0,
        "wmma m16n16k16 fp16\n× fp16 → fp32 accum\nBK == G: scales\nconstant per K-tile",
        C_OP, fontsize=8.5)
    ...
    box(ax, 0.3, 1.4, 6.0, 1.4,
        ...
        C_REG, fontsize=9)
    arrow(ax, (6.3, 2.1), (8.1, y4), color="#666", linewidth=1.0,
